...
При использовании метода **loadModalAsync** модальное окно может использовать динамические данные, в которых нет необходимости при первоначальном рендере, для исключения возможных ошибок необходимо делать проверку на существование динамических переменных. 
Например в шаблонизаторе Blade это выглядит так:  ```{{ Variable ?? '' }}```
